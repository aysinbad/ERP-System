...
Version:        1.6.0 (تصحيح أخير: توحيد priceConfidenceState في كل الملف)
Status:         In Review
...
> هذا القسم يصف التفاصيل التقنية لعرض السعر الاسترشادي كمرجع اختياري وتسجيله كـ metadata للتدقيق عند عرضه بنجاح أثناء إنشاء البروفورما. المرجع الكامل لما يرى / ما لا يرى: `CRM.md` Business Rules #12 و#13.
...
### Price Guidance Flow قبل إنشاء البروفورما
...
هل المستخدم يملك pricing.viewSuggestedPrice؟
        │
   لا ──► لا عرض لـ Price Guidance Panel → المتابعة بسعر يدوي (بدون أي guidance record)
        │
   نعم  ▼
محاولة حساب السعر الاسترشادي لكل منتج في الفرصة
        │
        ▼
هل نجح الحساب والعرض؟ (وإلا: PRC_NO_COST_DATA)
        │
   لا ──► لا يُنشَأ أي PriceGuidanceRecord → عرض تنبيه (CRM_PROFORMA_GUIDANCE_UNAVAILABLE)
        │        → المتابعة بسعر يدوي دون أي عائق
        │
   نعم  ▼
...
المستخدم يُدخَل unitPrice الفعلي لكل بند (مستقل عن السعر الاسترشادي)
        │
        ▼
لكل بند عُرِض له سعر استرشادي بنجاح: تسجيل PriceGuidanceRecord واحد
(سجل واحد لكل بند — metadata للتدقيق فقط، لا يُعرَض للعميل)
        │
        ▼
عند الحفظ: هل unitPrice يُخالف سياسةً؟
(هامش أقل من الحد / خصم خارج الحد / تكلفة قديمة تشترط الاعتماد / Cost Override)
        │
   ...
   نعم ──► فحص pricing.approvePriceException
        ...
        │
        ▼
تنفيذ التحويل (proforma.items[].unitPrice هو السعر التجاري الفعلي)
```

**ملخص القاعدة الشرطية:**
```
إذا توفّر السعر الاسترشادي وعُرِض بنجاح:
    إنشاء PriceGuidanceRecord واحد لكل بند مَعروض
وإلا:
    لا تُنشأ أي سجلات guidance
    الاستمرار في إنشاء البروفورما بسعر يدوي مُدخَل
```

### Price Guidance Record (metadata للتدقيق — على مستوى كل بند، شرطي)

> Schema الكامل والقانوني في `Pricing_Implementation_Guide.md`. هنا فقط كيفية التخزين على البروفورما.
> **شرطي:** يُنشَأ فقط للبنود التي عُرِض لها سعر استرشادي بنجاح. لا يُنشَأ عند غياب الصلاحية أو عدم توفر السعر.
...
| الكود | السياق | HTTP |
|---|---|---|
| `PRC_PRICE_EXCEPTION_REQUIRED` | حفظ بروفورما بسعر يستوجب اعتماد استثناء تسعير | 403 |
| `CRM_PROFORMA_GUIDANCE_UNAVAILABLE` | خدمة التسعير أرجعت `PRC_NO_COST_DATA` — CRM يلتقطها ويحوّلها لتنبيه غير حاجب | 200 + warning |

> **ملاحظة حاسمة:** `PRC_NO_COST_DATA` خطأ **داخلي لخدمة التسعير فقط** (موثَّق في `Pricing_Implementation_Guide.md`) — **لا يظهر مباشرة لواجهة CRM ولا يحجب إنشاء البروفورما أبداً**. CRM يلتقط هذه النتيجة ويحوّلها دائماً إلى `CRM_PROFORMA_GUIDANCE_UNAVAILABLE` (200 + warning)، ويستمر المستخدم بسعر يدوي. غياب السعر الاسترشادي لا يُنتج أبداً استجابة 422 من مسار إنشاء البروفورما في CRM.
...
- عرض السعر الاسترشادي بنجاح وتسجيل `PriceGuidanceRecord` عند توفر الإرشاد *(يُسجَّل عبر `pricing.price_guidance_recorded` في Pricing Audit — انظر `Pricing_Implementation_Guide.md`)*.
...
| `crm.opportunity.converted_to_proforma` | نجاح التحويل لبروفورما | `oppId, proformaId, custCode` |
...
>
> **فصل الاهتمامات:** بيانات السعر الاسترشادي (`suggestedPriceAtDecision`, `priceConfidenceState`) لا تُحمَل في `crm.opportunity.converted_to_proforma` — تُسجَّل عبر `pricing.price_guidance_recorded` (حدث منفصل، موثَّق في `Pricing_Implementation_Guide.md`). الحدث هنا يوثّق حقيقة تجارية واحدة فقط: نجاح التحويل.
