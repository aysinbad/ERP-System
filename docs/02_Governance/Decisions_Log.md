...
Last-Updated:   2026-07-24 (إضافة ADR-021/022/023)
```
...
| **ADR-021** | **مدخلات التقييم الثابتة — تجميد كل قيمة تدخل الدفتر لحظة الترحيل** *(Alias: `ACC-B`)* · يعتمد على ACC-A · يحسم بُعد RFC-ACC-001 القيمي | **Proposed** | **2026-07-24** | `ADR/ADR-021.md` |
| **ADR-022** | **العكس بدل الحذف — تصحيح الأحداث المُرحَّلة بقيود عكسية لا بمحو** *(Alias: `ACC-C`)* · يعتمد على ACC-A + ACC-B | **Proposed** | **2026-07-24** | `ADR/ADR-022.md` |
| **ADR-023** | **قفل الفترة المركزي — خدمة واحدة تحرس كل مسار كتابة محاسبي** *(Alias: `ACC-D`)* · يعتمد على ACC-A · أساس تبعي لـACC-E | **Proposed** | **2026-07-24** | `ADR/ADR-023.md` |
...
> **ADR-021 = ACC-B · ADR-022 = ACC-C · ADR-023 = ACC-D — كلها `Proposed`، صدرت في جلسة 6.2.** `ACC-E` **لم يُنشأ — محجوب بقرار Product Owner (OQ-6.A)**؛ جلسة 6.1 أثبتت `No dedicated settlement or consumption path found for account 2017`.
...
- **الحساب 2017** (عمولات مبيعات مستحقة): **ما ورك-فلو التسوية المنضبط الذي يجب تصميمه للحساب 2017؟** جلسة 6.1 وجدت `No dedicated settlement or consumption path` بعد بحث موجَّه ⇒ 🔴 **يحجب `ACC-E` وحده** (OQ-6.A · Product Owner)، ولا يؤثر على ADR-020 ولا `ACC-B/C/D`.
