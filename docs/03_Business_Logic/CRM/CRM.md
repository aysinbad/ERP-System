...
قبل تنفيذ التحويل، يُتاح للمستخدم (بصلاحية `pricing.viewSuggestedPrice`) اختيار سعر البند من:
- السعر الاسترشادي المحسوب من Pricing Engine.
- سعر يدوي (يخضع لفحص الاستثناء إن كان خارج السياسة).

السعر المختار يُحفَظ كـ **Snapshot ثابت** على البروفورما — البروفورما لا تُعيد حسابه لاحقاً. (تفاصيل تقنية في `CRM_Implementation_Guide.md` قسم Pricing Integration.)
...
عند تحويل فرصة ناجحة لبروفورما، يستطيع المستخدم (بصلاحية `pricing.viewSuggestedPrice`) اختيار سعر البند من السعر الاسترشادي أو سعر يدوي.
...
4. إذا اختار المستخدم سعراً يدوياً خارج نطاق السياسة: يُطبَّق فحص `approvePriceException`.

**⚠️ ملاحظة الـ Prototype:** `crmOppToProforma` في الـ Prototype يستخدم `Product.price` الثابت. قاعدة #13 تحل محله في البناء الإنتاجي (OQ-1 مغلق).
